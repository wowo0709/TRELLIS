...
function modelviewer_window_template(item, panel_template, config) {
    let viewer_size = config && config.viewer_size || 500;
    let panel_size = config && config.panel_size || 300;
    let show_hotspots = config && config.show_hotspots || false;
    html = `<div class="x-row" style="align-items: stretch;">
                <div class="modelviewer-container" style="width: ${viewer_size}px; height: ${viewer_size}px;">
                    <model-viewer
                        ...
                        >`
    if (show_hotspots) {
        for (let i = 0; i < item.hotspots.length; i++) {
            html += `<button slot="hotspot-${i}" data-position="${item.hotspots[i].position.join(' ')}">${item.hotspots[i].name}</button>`; 
        }
    }
    html += `</model-viewer>
                </div>
