...
from .components import CustomStandardDatasetBase, CustomTextConditionedMixin, CustomImageConditionedMixin
from .. import models


class CustomSparseStructureLatentVisMixin:
    def __init__(
        self,
        roots: str, 
        data_type: str, 
    ...
    def _loading_ss_dec(self):
        if self.ss_dec is not None:
            print("=> Pretrained sparse structure decoder already loaded.")
            return
        if self.ss_dec_path is not None:
            print("=> Pretrained sparse structure decoder path is given.")
...
class CustomSparseStructureLatent(CustomSparseStructureLatentVisMixin, CustomStandardDatasetBase):
    """
    ...
    def __init__(self,
        ...
        *,
        ...
        self.roots = list(map(lambda x: os.path.join(x, self.latent_model), self.roots))
    ...
    def get_instance(self, root, instance):
        latent = np.load(os.path.join(root, instance, 'ss_latent.npz'))
...
class CustomTextConditionedSparseStructureLatent(CustomTextConditionedMixin, CustomSparseStructureLatent):
    """
...
class ImageConditionedSparseStructureLatent(CustomImageConditionedMixin, CustomSparseStructureLatent):
    """
